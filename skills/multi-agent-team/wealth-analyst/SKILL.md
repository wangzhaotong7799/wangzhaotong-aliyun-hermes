...
| 6个赛道 | 18-21次（6赛道×2~3+2~4平台+2失败案例） | 同上 | ~100-150条记录（Exa API） | ✅ 2026-05-25, 2026-06-22, 2026-07-06, 2026-07-20 |
...
| 第2组 | 抖音/快手/视频号电商 + 失败案例 | 美妆/美食/母婴 + 失败案例 | 社交裂变CPS/短视频带货CPS/跨境CPS + 失败案例 |
...
**⚠️ 并行采集坑点（2026-07-20 新增）：子Agent工作目录 ≠ 父Agent工作目录**
- delegate_task 子Agent的 `data/` 相对路径可能指向 `/tmp/data/` 而非父Agent期望的 `/root/data/`
- 表现：子Agent写入 `data/tianwang_group1_20260720.md` 后，父Agent在同路径找不到该文件
- **对策**：在 context 中显式指定 `cd /root && python3 /tmp/script.py`，或采集完成后用 `find / -name "tianwang_group*_$(date +%Y%m%d).md"` 定位文件再 `cp` 到期望目录
- **更好对策**：采集脚本的 output_path 使用绝对路径（`/root/data/tianwang_group1_...`），避免相对路径歧义
...
**⚠️ 年份验证坑点（2026-07-20 新增）：Track名模糊匹配**
- 天网采集的赛道名（如"平台政策(抖音)"）可能不匹配正式赛道名（如"直播电商(抖音)"）
- 年份验证脚本需要建立别名映射表（TRACK_ALIASES），将"平台政策(抖音)"→"直播电商(抖音)"、"平台政策(跨境)"→"TikTok Shop"等
- **对策**：年份验证脚本中使用模糊匹配解析 track_raw → 标准赛道名，而非精确字符串匹配
- 模板脚本已更新包含 alias 映射，自定义脚本需同步更新
...
### 🛟 第一轮通过率特别低（<30%）时的多策略救援方案（2026-06-29 经验）

当第一轮 Exa 搜索脚本未捕获 `publishedDate` 字段，导致大量条目（>50%）因"无日期标注"被剔除时，**不要接受低覆盖率**。执行多策略救援：

**策略 A — URL 隐式日期检查（快速修复，0 API 成本）**
对未通过条目，检查 URL 中是否包含 `20[12]\d(0[1-9]|1[0-2])\d{2}` 模式的日期编码：
- `21jingji.com/article/20260222/herald/` → 2026年2月22日 ✅
- `people.com.cn/n1/2026/0514/` → 2026年5月14日 ✅
- 这种模式下，年份信息直接在 URL 路径中，无需 API 调用即可判定

**策略 B — 36kr ID 启发式（快速修复，0 API 成本）**
36kr 文章 ID = `36kr.com/p/{id}`，ID 区间映射：
- ID ≥ 37,000,000,000 (≥37亿) → 2026年（高置信度）
- ID 27-36亿 → 2025年
- 可用脚本批量判定所有 36kr 来源的无日期条目

**策略 C — 重跑 Exa 搜索 + 捕获 publishedDate（有 API 成本）**
若策略 A+B 后通过率仍 < 50%，重跑聚焦搜索并在脚本中显式捕获 `publishedDate` 字段：
```python
...
**策略 D — 平台首页/官方文档判定（0 API 成本，2026-06-29 新增验证）**
当 unknown 条目来自平台官方站点或服务商首页，且内容为当前运营信息时，可安全判定为当前年份：
- `union.meituan.com` / `union.jd.com` / `jinbao.pinduoduo.com` → 平台联盟首页 ✅
- `taobao.com/market/common/taoke-intro.php` / `alimama.com` → 官方推广页 ✅
- `affiliate-program.amazon.com` / `ads.tiktok.com` → 官方联盟首页 ✅
- `kwaixiaodian.com/cps` / `cps.ym.qq.com` / `youzan.com` → 服务商运营中 ✅
- `help.shopee.com` / `seller.shopee.sg` → 帮助中心（当前版本） ✅

**判定逻辑**：维护一个 `platform_homepage_domains` 列表，URL 匹配任一域名即标记 `data_year=2026`，`year_source=platform_current`。该列表需按域扩展：电商域包含 `tmall.com`/`taobao.com`，自媒体域包含 `douyin.com`/`kuaishou.com` 等。

**策略 E — 正文文本2026检测（0 API 成本，2026-06-29 新增验证）**
当 truncate 文本内容中明确包含"2026"（如 `2026年1月`、`2026/03/17`、`有效期至2026`），且 URL/标题均无日期时，可判定为当前年份。注意排除干扰：
- 文本中"2025"和"2026"同时出现（对比类文章，需人工复核）
- 纯数字串误匹配（如订单号、价格数字含"2026"）
- 安全阈值：仅在文本明确提及"2026年"或"2026/YYYY"格式时判定

**修复优先级**：publishedDate(最高) > URL日期 > 平台首页 > 36kr ID > 标题 > 正文文本(最低)

**实战案例（2026-06-29，CPS联盟营销域 6 赛道）**：
- 首轮 119 条，初始 65 条通过（54.6%），25 条无日期，29 条 <2026
- 策略 D（平台首页）修复了 **13 条**（union.meituan.com×3, union.jd.com×2, taokeniao.cn×2等）
- 策略 E（正文文本2026）修复了 **3 条**（Shopee affiliate page×2, 雨果网×1）
- 策略 B（36kr ID）修复了 **2 条**
- 综合通过率：85/119 = **71.4%**（CPS域教程/论坛类内容元数据质量低于自媒体域）

**实战案例（2026-06-29，自媒体域 8 赛道）**：
- 首轮 116 条，仅 52 条通过（44.8%），59 条无日期
- 策略 A（URL 日期模式）+ 策略 B（36kr ID 启发式）修复了 **51 条**
- 最终通过率：103/116 = **88.8%** ✅ 全部 8 赛道 ≥ 5 条有效数据

**跨域适用性**：策略 D（平台首页）在 CPS 域和电商域效果最好（大量官方联盟首页）；策略 A（URL 日期）在自媒体域最佳（新闻网站 URL 编码日期）。根据域类型选择主修复策略。

> 💡 **复用模板**：`templates/deep_year_recovery.py` 提供了一个完整的多策略深层恢复脚本，包含平台首页判定+36kr ID+文本2026检测。修改 `DOMAIN_CONFIG` 和 `INPUT` 路径即可复用。
...
| 电商(10赛道) | 100 | 92 | 8 | **92.0%** | 2026-06-22 | Exa 16次搜索(truncate=1200) |
| 电商(10赛道) | ~105 | ~95 | ~10 | **~90%** | 2026-07-20 | Exa 18次(并行3路,truncate=1200), publishedDate高可用 |
...
| CPS联盟营销(6赛道) | 119 | 85 | 34 | **71.4%** | 2026-06-29 | Exa 17次(truncate=1200)+多策略救援(平台首页+文本2026+36kr ID) |
| CPS联盟营销(6赛道) | 108 | 79 | 29 | **73.1%** | 2026-07-06 | Exa 17次(truncate=1200)+并行采集 |
| CPS联盟营销(6赛道) | 147 | 100 | 47 | **68.0%** | 2026-07-20 | Exa 19次(truncate=1200)+手动模式 |
...
**⚠️ 实践确认（2026-06-29）**：Exa 的 `publishedDate` 字段在 自媒体域 和 电商域 可用性高，但**搜索脚本必须主动捕获该字段**，否则年份验证会严重低估通过率。搜索脚本的 Exa 响应处理中应包含 `r.get('publishedDate', '')` 读取逻辑。当前模板脚本已包含该字段，但早期自定义脚本常遗漏此字段，导致大量 2026 年文章因"无日期标注"被误剔除。
...
| 电商周报(10赛道,并行采集+联合手动模式) | 10 | 17(Exa) | ~310K | ~11.6K (188行报告) | ~4min |\n| 电商周报(10赛道,2026-07-20) | 10 | 18(Exa并行3路) | ~320K | ~19K (373行报告) | ~5min |\n| CPS周报(6赛道,手动Exa+二轮聚焦,truncate=1200) | 6 | 21(Exa) | ~350K | ~14K (220行报告) | ~4min |
| CPS周报(6赛道,手动Exa+多策略救援,truncate=1200) | 6 | 17(Exa) | ~310K | ~16K (288行报告) | ~4min |
...
>
> **2026-06-29 最新（CPS联盟营销域）**：17次 Exa 搜索（6赛道×2+3政策+2失败），truncate=1200。119条原始→85条有效（71.4%，含平台首页+文本2026+36kr ID 多策略修复）。报告288行/16KB。社交裂变CPS登顶(6.35)，跨境CPS垫底(5.05)。全部6赛道≥5条有效数据。策略D（平台首页）和策略E（正文文本）首次验证有效。
>
> **2026-06-29 最新（自媒体域）**：17次 Exa 搜索（并行3路：短剧/本地/教育+平台政策/美妆/美食/母婴+失败/数码/健身+补充），truncate=1200。116条原始→103条有效（88.8%，含URL日期+36kr ID补充修复）。报告314行/18KB。短剧登顶(6.00)，美妆垫底(4.55)。多策略救援（零API成本）将无日期数据从59条降至8条。全部8赛道≥5条有效数据。
>
> **2026-06-29 最新（电商域）**：17次 Exa 搜索（并行3路：5+6+6赛道），truncate=1200，115条→107条有效（93.0%）。报告188行/11.6KB。视频号电商登顶(6.95)，抖音直播跌至末位(4.00)。并行采集+联合手动模式下报告行数低于全委托模式（188 vs 308），属合理优化——节省阶段间重复上下文格式开销。\n>\n> **2026-06-22 最新**：16次 Exa 搜索（10赛道+3政策+2失败+1补充），truncate=1200，100条→92条有效（92.0%）。报告308行/16KB。视频号电商登顶(8.15)，即时零售因旧数据偏多仅4条有效需标注置信度低。
>
...
>
> **2026-07-20 最新（电商域）**：18次 Exa 搜索（并行3路：6+6+6），truncate=1200，~105条原始→~95条有效（~90%）。报告373行/19KB。视频号电商蝉联第一(8.00)，即时零售跃居第二(7.30)，拼多多白牌第三(6.80)，直播电商(抖音)跌至末位(4.80)。全部10赛道≥5条有效数据。Exa publishedDate 字段持续高可用（电商域第6次验证）。新增坑点：子Agent工作目录差异导致文件路径错位需手动修复。
>
