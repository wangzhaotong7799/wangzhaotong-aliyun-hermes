...
## Venv 搬迁修复（移动项目目录后的必要步骤）

### 问题场景

将 Flask 项目目录从旧路径（如 `/root/projects/`）移动到新路径（如 `/workspace/projects/`）后，**venv 中的所有可执行脚本**（`gunicorn`、`pip`、`flask`、`pypinyin` 等）会因 **shebang 硬编码** 而全部失效：

```bash
# 症状：gunicorn 报 bad interpreter
$ /workspace/venv38/bin/gunicorn
/usr/bin/bash: /workspace/venv38/bin/gunicorn: /root/projects/.../venv38/bin/python3.8: bad interpreter: No such file or directory
```
...
```bash
find venv/bin -type f -exec grep -l '#!/old/path' {} \; | while read f; do
    sed -i 's|#!/old/path/venv/bin/python3.x|#!/new/path/venv/bin/python3.x|g' "$f"
done
# 验证
grep -rl '#!/old/path' venv/bin/ || echo "全部已修复"
```

#### 第 3 步：更新 systemd 服务（如果有）

```bash
# 更新 WorkingDirectory
sed -i 's|/old/path|/new/path|g' /etc/systemd/system/xxx.service
# 更新 ExecStart 中的全路径
sed -i 's|/old/path|/new/path|g' /etc/systemd/system/xxx.service

systemctl daemon-reload && systemctl restart xxx.service
systemctl status xxx.service --no-pager -l
```

#### 第 4 步：验证服务正常

```bash
curl -s -o /dev/null -w "HTTP %{http_code}" http://SERVER/
# 期望输出: HTTP 200
```

### 预防措施

1. **不要直接 mv 移动 venv** — 最佳实践是在新位置重建 venv
2. 如果必须移动，提前记录需要替换的所有路径，一次性批量替换
3. 移动后检查：gunicorn shebang、pip shebang、系统服务文件、crontab、脚本中的硬编码路径
