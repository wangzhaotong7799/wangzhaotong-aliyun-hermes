...
**吸收自 `flask-web-preload-lazy-load`**（2026-05 合并）：该技能补充了 `init()` 中默认页直接渲染而非走 `switchPage` 完整逻辑的关键优化、`data-rendered` 防重复渲染属性、以及按项目经验评估的性能对比（首页 JS 加载从 8 个脚本降至 2 个）。细节已整合到下方懒加载实现模板中。
