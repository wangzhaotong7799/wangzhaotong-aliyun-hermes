...
- **web_verified — verify an undated news-roundup by web-searching a dated fact inside it** (validated 2026-08-31, CPS域): undated "每日热点/跨境早报/新闻盘点" articles (e.g. `ikj168.com/46062` 跨境精选热点) carry NO URL date and NO publishedDate, but their body cites concrete, independently checkable facts (公司上市日、政策生效日、平台上线日). Run `web_search` on that fact to pin the article's year — zero API cost, no page fetch. Example: the roundup mentioned "千岸科技成功上市"; web_search confirmed 千岸科技 listed on 北交所 2026-07-29 → `data_year=2026` ✅. **Applicability rule**: the fact must be specific and checkable (IPO date / policy effective date / platform launch); vague industry narrative is NOT enough — 宁缺毋滥. Priority order stays: publishedDate > URL date > platform_current > title > text; this slot in before final reject.
...
**Delegated parallel collection is equally validated** (2026-08-31 电商域 10-track run): when `delegate_task` is available, the simplest reliable variant is 3 × `delegate_task(tasks=[...])` leaves, each told to run one pre-written group script and verify the output file exists (report back the head-stats). Key enabler — make each script FULLY self-contained so subagents need zero setup: (1) inline API-key load from `~/.hermes/.env` inside the script (no `source` step for the subagent), (2) absolute output paths (avoids subagent-cwd mismatch), (3) per-track stats printed at the end so the subagent can echo numbers back. Result: 24 Exa searches → 163 raw → 155 valid (95.1%), ~37s wall time, zero fixes. Reuse the previous run's group scripts as templates (copy + swap TRACKS + output filename + `collected_at`). See `references/ecommerce-20260831-session.md`.
...
`references/ecommerce-20260831-session.md` — 电商域 10-track run: DELEGATED 3-group parallel collection (fully self-contained scripts, no source/cp fixes), 24 Exa searches → 163 raw → 155 valid (95.1%), scoring ranking + Q2-2026 policy signals (TikTok Shop 美区登顶/503亿美元, TEMU垫底 欧盟3欧元关税+美区7:3流量新政, 快手停止披露电商GMV), read_file-binary re-confirmed on subagent side (2026-08-31).
`references/cps-20260831-session.md` — CPS 6-track run: single-script 18-search pattern, 120 raw → 103 valid (85.8%, CPS-domain best — publishedDate now 39% dominant in CPS, reversing the old "CPS publishedDate rare" guidance), web_verified recovery technique (ikj168 confirmed via 千岸科技 2026-07-29 IPO), 本地生活CPS old-tutorial-mixing pitfall (线报酷/九头蛇/开网店网/淘客秀 old SEO posts must be rejected by body date tags), scoring ranking + new policy signals (豆包AI渠道独立计费 12-18%, 微信推客API开放, 京东联盟达人补税, TikTok欧洲POP领航计划) (2026-08-31).
